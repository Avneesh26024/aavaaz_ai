...
- Facial engine is a singleton to avoid reload cost.
- SenseVoice SER uses per-session engine instances but shared prewarmed model weights.
- TTS streaming is backend-driven; frontend only handles playback.

## Suggested Next Steps

- Add a setup section with exact install and run commands.
- Document expected Redis configuration for Channels.
- Add a deployment guide (Daphne + ASGI + Redis).
